...
**Stable public URL (as of 2026-04-24):** `https://pg.rapid-raccoon.com` — named Cloudflare Tunnel `party-games` on this laptop → `localhost:8080`. Replaces the old rotating trycloudflare.com quick tunnels. Cast Console receiver URL is now `https://pg.rapid-raccoon.com/receiver` (set 2026-04-24). Don't cycle the Cast Console URL any more — stable now, no more restarting the propagation clock. Setup script: `~/bumper2bumper/scripts/cloudflare-create-named-tunnel.sh`. Config at `~/.cloudflared/config.yml`; tunnel runs by hand (`cloudflared tunnel run party-games`) until Party Games relay moves to an AWS server (TODO in migration plan).
