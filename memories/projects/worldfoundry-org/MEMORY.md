...
- [Layout props vs path sniffing](feedback-layout-props-vs-path-sniffing.md) — prefer `gallery` prop on Base.astro over `Astro.url.pathname` for page-specific layout variations; path checks still valid for active-nav and middleware cases
- [WorldFoundry casing](feedback-worldfoundry-casing.md) — when a space won't fit, camelcase as "WorldFoundry" (not "Worldfoundry"); brand convention
- [Colored checkmarks](feedback-colored-checkmarks.md) — use ✅/❌ emoji (not plain ✓/✗) in reports/tables the user reads
- [Canonical not always biggest](feedback-canonical-not-always-biggest.md) — when consolidating duplicate scripts, audit each copy's unique features first; line count ≠ completeness
