...
- [Stitch MCP operational quirks](feedback_stitch_rate_limits.md) — keep seed prompts under ~400–600 chars; iterate via `generate_variants` to dodge rate limits
- [Lambda Function URL needs TWO permissions in Terraform](feedback_lambda_function_url_two_permissions.md) — Terraform-managed Function URL 403 AccessDeniedException → missing `invoked_via_function_url=true` permission (requires AWS provider ~> 6.0)
- [Don't block on remote operations](feedback_dont_block_on_remote_operations.md) — for deploys / TF applies / cert issuance, use run_in_background or ScheduleWakeup and report status periodically; never sleep-loop the conversation
- [No Claude Co-Authored-By trailer](feedback_no_claude_coauthor_trailer.md) — a local hook rejects the `Co-Authored-By: Claude …` trailer in commits; omit it
