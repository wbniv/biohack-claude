...
## Project
- [project_config_yaml_mystery_roundtrip.md](project_config_yaml_mystery_roundtrip.md) — Active watch: config.yaml roundtrip mutation mystery — watch for recurrence
- [project_hyderabad_future_home.md](project_hyderabad_future_home.md) — Mumbai (ap-south-1) is current infra region; Hyderabad (ap-south-2) is migration target once feature parity lands (same price, lower ping)

## Generated
- [memory-visualization.md](memory-visualization.md) — Auto-generated memory summary — re-run `task memory-viz` after memory changes

## Grafana Access

Grafana admin password is in AWS SSM: `/parkingspace/prod/GRAFANA_ADMIN_PASSWORD`

Retrieve: `aws ssm get-parameter --name "/parkingspace/prod/GRAFANA_ADMIN_PASSWORD" --with-decryption --profile terraform --region us-west-2 --query 'Parameter.Value' --output text`

CloudWatch datasource uid: `cloudwatch` — metrics API works, logs API not authorized.
